...
The Laplace mechanism is one of the most commonly used methods for implementing 
Differential Privacy (DP) in numerical data queries. For this project, we employed
the Laplace mechanism, which provides $(\epsilon, 0)$-differential privacy. The noise
added to query results is sampled from a Laplace distribution defined as:
$X \sim \text{Laplace}(0, \Delta f / \epsilon)$.

To implement Differential Privacy (DP) in this project, the target function was 
defined as a linear regression model that estimates the relationship between 
the log revenue (`lnrev`) and variables such as `budget`, `popsqrt * genre`, 
`o_lang`, and `vc`. This function returns the coefficients of the regression, 
including the intercept, which form the query output to be privatized using 
the Laplace mechanism. By applying noise to the coefficients, the mechanism 
ensures that the model results do not reveal sensitive details about 
individual data points in the dataset.

The sensitivity of the query, denoted as $\Delta f$, measures the maximum change
in the regression coefficients caused by the addition or removal of a single data 
point. For this analysis, $\Delta f$ was set to $1$. This ensures that the magnitude
of the noise added to each coefficient is proportional to its sensitivity.

The privacy budget $\epsilon$ was set to $1$. A smaller value of $\epsilon$ 
would increase the amount of noise, providing stronger privacy protection but 
reducing the accuracy of the regression coefficients. Conversely, a larger value
of $\epsilon$ would result in less noise, improving accuracy but weakening 
privacy guarantees. 
